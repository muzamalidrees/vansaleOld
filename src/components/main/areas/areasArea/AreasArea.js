...
import { setAreas } from '../../../../actions/area-actions'
import { connect } from 'react-redux'


class AreasArea extends Component {
    componentWillMount() {
        fetch('/getAllAreas',
        )
            .then((res) => res.json())
            .then((json) => {
                console.log(json)
                this.props.dispatch(setAreas(json.data))
    ...
    callTable = () => {
        if (this.state.showTable) {
            return <SearchResultsTable
                ASearchResults={this.props.areas}
...
const mapStateToProps = (store) => {
    return {
        areas: store.areasReducer
    }
}

export default connect(mapStateToProps)(AreasArea)